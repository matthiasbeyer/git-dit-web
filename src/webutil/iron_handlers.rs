...
pub fn index(_: &mut Request) -> IronResult<Response> {
    ...
    resp.set_mut(Template::with(
        include_str!("templates/lists/issue_list.hbs"),
        data,
    )).set_mut(status::Ok);
    Ok(resp)
}
...
pub fn show_issue(req: &mut Request) -> IronResult<Response> {
    ...
    let mut resp = Response::new();
    let ref message_id = req.extensions
        .get::<Router>()
        .unwrap()
        .find("message_id")
        .unwrap_or("/");
    let ref order = req.extensions
        .get::<Router>()
        .unwrap()
        .find("order")
        .unwrap_or("/");
    ...
    resp.set_mut(Template::with(
        include_str!("templates/lists/show_issue_tree.hbs"),
        bt_data,
    )).set_mut(status::Ok);
    Ok(resp)
}
...
pub fn new_cat_tag_handler(req: &mut Request) -> IronResult<Response> {
    use webutil::tags::*;
    let ref tag = req.extensions
        .get::<Router>()
        .unwrap()
        .find("tag")
        .unwrap_or("/");
    let ref target = req.extensions
        .get::<Router>()
        .unwrap()
        .find("target")
        .unwrap_or("/");
    new_cat_tag(String::from(*tag), String::from(*target));
...
pub fn reply_message_handler(req: &mut Request) -> IronResult<Response> {
    ...
    resp.set_mut(Template::with(
        include_str!("templates/lists/submit_reply.hbs"),
        data,
    )).set_mut(status::Ok);
    Ok(resp)
}
