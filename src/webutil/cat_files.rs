...
pub fn list_issues() -> IssueList {
    ...
    // descending order
    let mut sort_key: Box<FnMut(&Issue) -> git2::Time> =
        Box::new(|ref issue| issue.initial_message().unwrap_or_abort().time());
...
pub fn issue_to_json(issue: Issue, order: &str) -> IssueToJson {
    ...
    let mut messages: Vec<git2::Commit> = issue.messages().unwrap().map(|s| s.unwrap()).collect();
...
pub fn new_issue(hashmap: &HashMap<String, Vec<String>>) {
    ...
    repo.create_issue(&author, &committer, new_message.trim(), &tree, Vec::new())
        .unwrap();
}
...
pub fn new_message(hashmap: &HashMap<String, Vec<String>>) -> Oid {
    ...
    issue
        .add_message(&author, &committer, message.trim(), &tree, parent_refs)
        .unwrap_or_abort();
